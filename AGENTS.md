...
- **Agents** (`.opencode/agent/` · `.claude/agents/`): code-reviewer, explorer, orchestrator, python-engineer
- **Commands** (`.opencode/command/` · `.claude/commands/`): add-language, adr, build, checkpoint, component, contract-check, docs-sync, golden, golden-approve, lint, new-contract-rule, orient, pipeline, review, strangler-step, test, verify-work
- **Skills** (`.opencode/skill/` · `.claude/skills/`): data-contracts, gate-model, golden-debug, golden-testing, pipeline-map, polyglot-boundary, python-conventions, skill-creator, two-plane-memory
