...
- **Commands** (`.opencode/command/` · `.claude/commands/`): add-language, adopt, adr, agree, build, checkpoint, component, contract-check, docs-sync, fan-out-synthesize, flow, impact, lint, new-contract-rule, orient, refresh-memory, review, test, verify-work
- **Skills** (`.opencode/skill/` · `.claude/skills/`): brownfield-adoption, context-budget, data-contracts, fan-out-synthesize, harness-author, polyglot-boundary, python-conventions, two-plane-memory
