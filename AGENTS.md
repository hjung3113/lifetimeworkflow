...
- **Commands** (`.opencode/command/` · `.claude/commands/`): add-language, adopt, adr, agree, build, checkpoint, component, contract-check, docs-sync, fan-out-synthesize, golden, golden-approve, lint, new-contract-rule, orient, refresh-memory, review, test, verify-work
- **Skills** (`.opencode/skill/` · `.claude/skills/`): brownfield-adoption, context-budget, data-contracts, fan-out-synthesize, golden-debug, golden-testing, polyglot-boundary, python-conventions, skill-creator, two-plane-memory
