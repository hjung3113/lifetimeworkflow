...
- **Sorted keys** in all emitted JSON (`json.dumps(sort_keys=True)`); preserve authored key order only where semantically required (the `bash` last-wins matrix — emit its keys in the AUTHORED insertion order, NOT sorted, because order is semantic; document this exception).
