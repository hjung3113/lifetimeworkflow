# Requirements Archive: v2.3 Contract Graph, Brownfield Adoption, Living Docs
...
- [x] **DOCSUP-06** *(NEW+REUSE, Phase 29)*: 하나의 얇은 `/docs-update` command와 `docs-upkeep` skill이 결정론적 큐를 읽어 bounded 사람-doc 편집 또는 정확한 review disposition을 draft하고 기존 review/verify 흐름으로 라우팅하며, accepted ADR·`docs/reference/**`·`.memory/derived/**`·contracts·golden을 구조적으로 제외하고 두 런타임에 왕복한다.
- [x] **DOCSUP-07** *(NEW+REUSE, Phase 29)*: 정상 사람-review 변경이 최고 위험 기존 문서와 adoption runbook의 binding을 seed하며, `/adopt`은 registry/ledger 항목을 제안할 수 있으나 inferred ownership은 미해결로 남기고 명시적 review 없이는 green이 될 수 없다.
...
| DOCSUP-06 | Phase 29 | Complete — RAT-1 discharged 2026-07-22 |
| DOCSUP-07 | Phase 29 | Complete — RAT-1/RAT-3 discharged 2026-07-22 |

> **Why DOCSUP-06/07 were held open, and what closed them.** Both were built, emitted to both
> runtimes and gate-tested from the end of Phase 29 — `exclusion_reason()` with per-class deletion
> proofs, `/docs-update` + `docs-upkeep` round-tripping byte-identically (100 artifacts, porcelain
> empty), and 14 integration tests proving `/adopt` can propose a binding and cannot make one green.
> What was missing was never code: `uv run python -m tools.docs_guard` exited 1 with 6 ×
> `broken-binding` because `docs/.docs-review-ledger.toml` did not exist, and that file is
> human-only by construction (`tools/hooks/ledger_guard.py` honours NEITHER `GOLDEN_APPROVE_HUMAN`
> NOR `HARNESS_DEV_BYPASS`). Marking them complete before that would have been the self-blessing the
> requirements exist to forbid.
>
> **Closed 2026-07-22.** The human authored the first ledger outside an agent session (`c32c08d`)
> and ratified ADR-0010 (`ad4e339`). `docs_guard` moved exit 1 → exit 0, 8/8 bindings FRESH,
> `uncovered_max = 7`. Phase-29 re-verification additionally drove the loop green → red → green in a
> throwaway worktree (advisory edit = `STALE_ADVISORY`, exit 0; required edit = `STALE_REQUIRED`,
> exit 1; revert = exit 0), and re-confirmed `ledger_guard.decide()` still DENYs after the ledger
> existed — under `{}`, `GOLDEN_APPROVE_HUMAN`, `HARNESS_DEV_BYPASS`, and both together. Evidence
> rows: `.planning/v2.3-MILESTONE-AUDIT.md` and `29-VERIFICATION.md`.
