...
status: not_passed_at_close
score: 3/4 criteria substantiated from artifacts; 1 disproven by the phase's own review
overrides_applied: 0
...
> artifact that already existed** — the six PLAN/SUMMARY pairs, `27-REVIEW.md`, the approved
> contemporaneous `27-VALIDATION.md`, `deferred-items.md`, and the Phase 27/27.1/27.2 entries in
> `.planning/milestones/v2.3-ROADMAP.md`. Where no such artifact exists, the row says so and is
> counted as a gap. Nothing here was inferred from reading today's code.
>
...
|---|---|---|---|
| SC-1 | `.workflow/tasks/<id>/artifacts/adoption/<batch>/` batch가 안전하게 재개되고, 변경된 draft/ref/revision이 승인을 무효화한다. | ✓ SUBSTANTIATED (as recorded) | `27-01-SUMMARY.md` provides `batch_id_for()/create_or_resume_batch()/read_status()/update_status()`, with `<batch-id> = sha256(target_ref\|UTC-date)[:16]`, "deterministic per (target_ref, discover-day)" (D-02), and CAS-guarded `status.json`. `27-04-SUMMARY.md` provides `approval.py` and states "SC-1 full resume + invalidation cycle proven end-to-end, composing 27-01's batch.py with this plan's approval.py", with the pattern "Fresh-recompute invalidation: any single-axis change (draft/revision/ref) independently invalidates a prior approval, **proven by three separate tests holding the other two axes constant**". It records `test_sc1_full_resume_cycle -x` passing standalone. **`27-REVIEW.md` raised no finding against the invalidation logic** — it calls "the approval binding tuple's own internal logic […] well constructed". |
| SC-2 | `contracts/`·`docs/adr/`·`golden/` destination이 mutation 전에 거부되고, 비-헌법 apply가 atomic·collision-safe·idempotent하다. | ✗ **DISPROVEN at close** (first half) · ✓ substantiated (second half) | **The refusal half did not hold.** `27-03-SUMMARY.md` claims "Structural, hook-independent constitution-plane refusal proven by a zero-filesystem-call spy". `27-REVIEW.md` **CR-01** (`apply.py:74-84`) demonstrates the bypass: `refuse_if_constitution` glob-matches the raw destination string with no normalization, so `a/../contracts/x.json` and (on this repo's own case-insensitive APFS target) `CONTRACTS/x.json` are not denied. **CR-02** reports no destination confinement at all — an absolute-path destination "writes outside `target_root` entirely". **CR-03** reports `approval.check_valid()` "is called nowhere outside its own unit test", so the ADOPT-06 gate 27-04 was built for "is entirely decoupled from the write path". The review states none of the three was covered by any existing test. **The atomic/collision-safe/idempotent half is substantiated and was not disputed:** `27-REVIEW.md` § Summary calls "the atomic-write primitives (`atomic_create`/`_atomic_replace`), the batch CAS layout […] well constructed and match their stated contracts". |
...
| SC-4 | `/adopt` + `brownfield-adoption` skill이 두 런타임에 byte-identical 왕복(새 persona 없음, 모델 id 없음)한다. | ✓ SUBSTANTIATED (as recorded) | `27-06-SUMMARY.md` provides `harness/commands/adopt.md` + `harness/skills/brownfield-adoption/SKILL.md`, "EXPECTED_SKILLS widened to 12 entries; emitted command count bumped 23 -> 24", and "Both `.opencode/` and `.claude/` runtime trees carrying /adopt + brownfield-adoption byte-identically (SC-4)". It records `uv run pytest tools/harness_emit -q` → **47 passed** after the `.ambr` refresh, and notes the gate-theft-avoidance ordering explicitly ("re-ran the full `tools/harness_emit` suite to confirm all 47 tests green **before** touching the committed `.opencode`/`.claude` trees"). `27-05-SUMMARY.md` independently records `uv run python -m tools.harness_emit.generate && git diff --exit-code -- .opencode .claude` → "clean, no drift". **Caveat, not smoothed over:** no artifact records an explicit *no-new-persona* or *no-model-identifier* check for this phase. See Gaps G-2. |

**Score: 3 of 4 criteria substantiated from pre-existing artifacts. SC-2 disproven by the phase's
own review.**
...
| Full suite at phase close | (§ Next Phase Readiness) | 1096 tests green | `27-06-SUMMARY.md` |
...
| WR-01 | Warning | `_apply_marker_merge` does an unlocked read-modify-write; concurrent applies can race | Addressed in the 27.1/27.2 line — `27.2-VERIFICATION.md` SC-3 records a `fcntl.flock` mutual-exclusion control and a test proven to go red when `flock` is neutered |
| WR-02 | Warning | `check_valid` raises instead of returning `False` when a draft artifact is missing | Closed by Phase **27.2** — `27.2-VERIFICATION.md` SC-2 (`WR-06` in that phase's numbering), 12-row corruption table |
| WR-03 | Warning | `manifest.schema.json`'s `destination` has no path-shape constraint | **Open, by decision.** `27.2-VERIFICATION.md` § Residual Tracking records "`manifest.schema.json` still admits `"."`/`"./"`/`"a/"` (runtime-only closure per D-08)" |
| WR-04 | Warning | `cli.py`'s `apply` never re-validates `manifest.json` against its schema before use | **No artifact records a disposition.** See Gaps G-3. |
| IN-01 | Info | `apply.py` docstring's security claim is broader than what the code proves | Overtaken by the CR-01/CR-02 fixes in 27.1 |
| IN-02 | Info | `_recompute_draft_hash` hashes files in fixed order but never validates their `sha256`/schema shape | **No artifact records a disposition.** See Gaps G-3. |
...
- **G-2 — SC-4's "새 persona 없음, 모델 id 없음" half is unsubstantiated.** `27-06-SUMMARY.md`
  substantiates the byte-identical dual-runtime round-trip and the command/skill counts, but **no
  artifact records an explicit no-new-persona or no-model-identifier check for this phase.** Other
  phases record these as named gate rows; Phase 27 does not. The row is marked "(as recorded)" for
  that reason and must not be read as a verified negative.
- **G-3 — WR-04 and IN-02 have no recorded disposition anywhere.** Neither the Phase 27 artifacts
  nor 27.1's / 27.2's mention them. They are not known to be fixed and not known to be open; they
  are unaddressed in the record. Naming them here is the only thing this document can honestly do
  with them.
...
Two items are for a human to note rather than to act on:

1. **SC-2 was not met when Phase 27 closed.** This document records that as the phase's actual
   outcome. Whether the ROADMAP's Phase 27 checkbox should reflect it is a bookkeeping decision
   outside this document's authority.
2. **G-3's two undispositioned review findings (WR-04, IN-02)** may deserve a decision — even a
   recorded write-off — rather than continuing to sit unmentioned.
