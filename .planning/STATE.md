...
**Current focus:** v2.2 Adaptive Task Control Plane. **Phase 18 + 19 + 20 + 21 + 22 COMPLETE.** P18 (merged `e8cc23e`): task packet contracts + `.workflow/tasks/` + validator. P19 (merged `19c8afc`): deterministic risk router (`harness/risk-policy.toml` + `tools/risk_router`), escalate-only overlay slot, `/intake` command (both runtimes). P20 (merged `27cd67f`): atomic state manager (`tools/task_control` — flock+revision CAS, interrupted-write recovery), phase-oriented required-artifact gate, fail-closed `/phase-gate` + `context-attestation.json` (ratified `attestation.schema.json`). P21 (merged `1b4a611`): forgery-detecting evidence adapters (`tools/evidence` — wraps existing gates, records argv/exit/hash/status, gate-registry.json exact-argv binding, secret/PII refusal, HEAD-committed evidence+approval trust root at COMPLETE via the no-agent-commit invariant). P22 (merged `2793f8e`): immutable HANDOFF snapshot + gated fresh-session resume (`tools/handoff` — HEAD-committed trust root, full-validate fresh-session, working injector pointer-only within token cap, task-specific stop_condition, PII refusal) + `resume_gate` PreToolUse hook (revision-bound attestation, advance-only re-stamp on sanctioned transitions — blocks un-resumed mutations without deadlocking resumed work). TCP-01..15 validated. Execution: build = codex terra (medium, fast off) headless; review alternates fable↔sol; Claude verifies + merges. **Note:** codex/sol final output is content-filtered on secret/forgery/hook-exploit-shaped reviews (OpenAI cyber filter) — P21 fully and P22's confirmation re-review ran on fable instead (justified deviation; [[design-and-gsd-via-codex-sol]] item 8). Signed external evidence attestation deferred to Phase 23 (P21 D-10). **P22 deferred minors (non-blocking, for P23 negative-fixture suite):** (a) add un-resumed→transition→Write-deny regression test; (b) close `env VAR=1 git commit` / `git -C <dir>` prefix bypass in resume_gate `_MUTATING_BASH`. Next: Phase 23 (Lifecycle Evaluation + Docs + CI) — reviewer = sol (or fable if secret/PII-heavy); includes human-ratified lifecycle fixtures + structural ADR.
