...
last_updated: "2026-07-29T18:13:18.263Z"
...
- **[RESOLVED 2026-07-16 — Phase 14] ADR-0006's phantom "committed seed"** — corrected by a dated `## Errata` appended to ADR-0006 (`ff832ac`), landed via `HARNESS_DEV_BYPASS` (first real use of ADR-0007; token never forged; byte-hygiene held). NOT fixed by shipping a seed — fabricating one needs invented user feedback, the T-13-01 violation the phase exists to prevent. The errata states plainly that the empty active set is CORRECT.
...
- **[PHASE 15 — carry in] `test_coexist` count is now 20, and the `.opencode`/`.claude` trees still hold 19.** Phase 14 authored `harness/commands/agree.md` (source-only) and bumped `test_all_20_commands_emit_to_both_trees` (`fa1aea8`). Phase 15 owes the actual re-emit so the committed trees carry `/agree` too. **Root lesson for Phase 15: `_emit(tmp_path)` counts the runtime-neutral SOURCE, not the committed trees** — Phase 14's D-10 assumed the opposite and was wrong; the error survived the planner AND the plan-checker and only surfaced when the code ran.
- **[PHASE 15] The one sanctioned red test remains `test_projected_tree_matches_committed_snapshot`.** Baseline is **1 failed / 658 passed**. `0 failed` = someone updated the `.ambr` and stole this gate. Phase 15 owns settling it by actually re-emitting, never by `--snapshot-update`.
- **Pre-existing repo lint debt (not Phase 14's):** base commit already carries ~57 `ruff check` E-codes in `tools/` plus 2 format-check failures (`tools/hooks/tests/test_stdin.py`, `tools/polyglot_lint/lint.py`). Phase 14 touched none of those files. `ruff check` is not currently a CI gate — worth deciding whether it should be.
- **`HARNESS_DEV_BYPASS` is now ENABLED** in gitignored `.claude/settings.local.json` (`{"env":{"HARNESS_DEV_BYPASS":"1"}}`). Constitution writes from this dev session are allowed and emit a dev-only note; they are never labeled human-ratified. CODEOWNERS at merge is still the real gate. Remove the file to restore strict enforce-by-default locally.
- **⚠ SECURITY (outside this repo, unresolved):** `~/.codex/config.toml` stores a **plaintext GitHub OAuth token** (`gho_…`) under `[mcp_servers.github.env]`, for a server that is `enabled = false`. It is on disk in the user's home, so no repo gate can see it. **Recommend rotating that token** and injecting it via env/`gh auth` instead. Surfaced 2026-07-16 while verifying Codex model flags.

### Quick Tasks Completed

| # | Description | Date | Commit | Directory |
|---|-------------|------|--------|-----------|
| 260729-wdi | **Closed the v2.5 handoff leftovers PR #5 left open.** Shipped: the advertised suite size at `README.md:12,107` and `README.ko.md:60`, 982 → **881**, measured with `--collect-only` and verified by a *negative* grep (a positive count grep would pass while a missed site survived). Corrected the handoff's own framing: `contract_guard.py:55` puts `docs/adr/**` on `CONSTITUTION_GLOBS` alongside `docs/glossary.md`, so **three** of the four leftovers are human-gated, not one — the ADR was never agent-authorable. No token set, no `HARNESS_DEV_BYPASS`. Staged `HUMAN-APPLY.md` instead: the full ADR-0013 body (supersedes 0008, ratifies the keep-cited-targets rule), ADR-0008's two header lines, the index row, and the two glossary replacements, all copy-ready off-plane. Resolved D-14 as a finding — both cited targets are **live**, so ADR-0003 needs no correction; the real dangling refs are the six deleted paths in ADR-0008's own immutable Links section. | 2026-07-29 | 19ff9e7 | [260729-wdi-close-v2-5-handoff-leftovers-adr-superse](./quick/260729-wdi-close-v2-5-handoff-leftovers-adr-superse/) |
| 260722-1iq | **Constitution-plane enforcement drift — `docs/glossary.md`.** ADR-0001:48 (accepted, unsuperseded) declares a FOUR-member plane; the Phase-4 stack implemented three, so the glossary was agent-writable for two weeks while four documents said it was gated. Fixed the enforcement, never the ADR: `CONSTITUTION_GLOBS` + its deny message, permission matrix, CODEOWNERS, `AGENTS.md`, `gate-model` skill (re-emitted), and the stale "Phase-4 not built" fossil in `.memory/README.md` (its plane table was already correct and stayed). Literal path, never `docs/**` — a negative control reds a broad-glob "fix". Rode along: GEN-04 prose named 1 of 3 sanctioned instance-pointer exemptions; `lifecycle-eval-shadow-metrics` gained `risk_router/router.py`, the actual owner of its `lane_override` row. **Two prior Claude audits recommended the OPPOSITE repair**, citing ADR-0010 (`proposed`) over ADR-0001 (`accepted`); an external audit caught the inversion. 1480 → 1500 passed; 4 mutation proofs; `docs_guard` still exit 1 for the unchanged pre-ratification reason. | 2026-07-22 | 732a0d5, 6d74a50, 378f8ba, f98d63a | [20260722-glossary-constitution-plane-drift](./quick/20260722-glossary-constitution-plane-drift/) |
...
|----------|------|--------|-------------|
| repo config (merge authority) | **D-24 — CODEOWNERS is advisory on this repo. ACCEPTED AS A DOCUMENTED RESIDUAL, decided 2026-07-29 by the owner.** Measured: branch protection on `main` requires the `gate` check (strict) but carries **no** `required_pull_request_reviews` block, and `enforce_admins` is false — so CODEOWNERS auto-requests review without being able to block a merge. `require_code_owner_reviews=true` is **not** the remedy it looks like: GitHub forbids self-approval, so in a solo repo where the sole code owner is the PR author it cannot be satisfied without a second account or a per-merge admin waiver, and it was already considered and declined at v2.3 RAT-5. Accepting is consistent with ADR-0012 (CI and the merge are the authority) and with ADR-0013 clause (c) (no new enforcement) — adding a control here is exactly the ceremony v2.5 removed. Re-openable in v2.6 as a machine-side check on golden baseline diffs, which v2.5's no-growth constraint forbade and which is now unblocked. | accepted (closed) | v2.5 close |
...
Last session: 2026-07-29T18:12:25.973Z
