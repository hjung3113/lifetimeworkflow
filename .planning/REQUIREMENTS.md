...
- [ ] **PIPE-01**: `harness/project.toml`에 범용 **파이프라인-토폴로지 슬롯** — 각 컴포넌트의 id·stage·language 참조·edge 계약(consumes/produces)을 선언하는 순수 DATA 테이블(`[[components]]`/`[pipeline]`). `tools/harness_config/loader.py` passthrough + GEN-03-스타일 consistency 게이트. 예제 의존 0 (GEN-04 green).
