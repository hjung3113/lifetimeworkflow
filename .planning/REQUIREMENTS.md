...
- [x] **TOPO-01** *(NEW+REUSE, Phase 24)*: `contracts/harness/topology/` 아래 사람 승인 Draft 2020-12 스키마가 각 관계를 stable id + tracked contract reference + 정확히 하나의 authority endpoint + 1개 이상 dependent endpoint + 선택적 explanatory kind/labels로 정의하고, positive/negative fixture가 기존 contract-hash/drift ratification 경로를 통과한다.
