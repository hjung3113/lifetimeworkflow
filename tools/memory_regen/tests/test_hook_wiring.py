...
def test_session_start_has_exactly_four_groups(repo_root: Path) -> None:
    ...
    settings = json.loads((repo_root / ".claude" / "settings.json").read_text(encoding="utf-8"))
    groups = settings["hooks"]["SessionStart"]
    assert len(groups) == 4, f"expected 4 SessionStart groups (3 existing + injector), got {len(groups)}"
...
def test_injector_slot_references_memory_inject(repo_root: Path) -> None:
    ...
    commands = _session_start_commands(repo_root)
    assert any(INJECTOR_COMMAND in c for c in commands), "injector hook (memory-inject.sh) not wired"
...
def test_opencode_stub_authored_and_deferred(repo_root: Path) -> None:
    """The opencode adapter stub is authored, references the SAME contract, and is marked deferred."""
    stub = (repo_root / "harness" / "plugins" / "session-inject.ts").read_text(encoding="utf-8")
    assert "tools.memory_regen.inject" in stub, "opencode stub must consume the SAME assembler (D-01)"
    lowered = stub.lower()
