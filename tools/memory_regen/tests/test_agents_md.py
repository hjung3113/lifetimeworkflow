...
# After the 05-03 domain move (GEN-01), the instance's language-side package (incl. its per-package
# AGENTS.md) relocated under the log-parser example instance. The CORE per-package rules plane is now
# Python-only; the instance-language per-package rules live with the example (05-05 recasts the root map).
...
def test_root_agents_carries_map_goldenpath_contractfirst_lazyload(repo_root: Path) -> None:
    ...
    # the instance's language-side package relocated under the log-parser example instance, so it is
    # no longer a required CORE root-map member (05-05 recasts the root map to the template shape).
