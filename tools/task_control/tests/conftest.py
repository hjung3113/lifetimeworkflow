"""Import-path wiring for task_control tests.

Mirrors tools/lifecycle_eval/tests/conftest.py: task_control is a *virtual* uv-workspace member (not
...
repo root lets `from tools.task_control import ...` resolve.

Without this, the whole-repo `uv run pytest` still collects, because a sibling member's conftest
has already inserted the root by then — but an isolated `uv run pytest tools/task_control` errors at
...
# tests -> task_control -> tools -> repo root
