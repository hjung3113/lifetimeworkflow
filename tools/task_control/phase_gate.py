...
def phase_gate(
    ...
    *,
    ...
    root = (
        ...
        if repo_root
        else Path(_git(packet, "rev-parse", "--show-toplevel")).resolve()
    )
    refresh: list[str] = []
    try:
        state = show(packet)
    ...
    if state["current_ref"] != head and not (
        publication_parent == state["current_ref"]
        and _git(root, "rev-parse", "HEAD^") == state["current_ref"]
    ):
    ...
    # The same lane-discipline requirement the transition enforces, applied to the phase being
    # RESUMED: a fresh session must not inherit a green a fresh transition would have refused.
    refresh.extend(f"discipline: {item}" for item in missing_disciplines(packet, state["phase"]))
    ...
    diagnostics = (
        [f"orphan artifact: {path}" for path in orphaned]
        if state["phase"] not in {"VERIFY", "COMPLETE"}
        else []
    )
    ...
    try:
        ...
        records = (
            attestation.get("constraints")
            if isinstance(attestation.get("constraints"), list)
            else []
        )
    ...
    for constraint in task.get("constraints", []):
        ...
        if not source.is_file() or not record:
            ...
            continue
        if (
            record.get("source_path") != constraint["source_path"]
            or record.get("source_sha256") != constraint["source_sha256"]
            or sha256(source) != constraint["source_sha256"]
        ):
        ...
        if phase in {"VERIFY", "COMPLETE"} and (
            not isinstance(record.get("required_evidence_ids"), list)
            or not record["required_evidence_ids"]
        ):
