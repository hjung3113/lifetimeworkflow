...
def main(argv: list[str] | None = None) -> int:
    ...
    command = commands.add_parser("transition"); command.add_argument("task_dir", type=Path); command.add_argument("target"); command.add_argument("--expected-revision", type=int, required=True); command.add_argument("--next-action"); command.add_argument("--current-ref")
    ...
    command = commands.add_parser("refresh-ref"); command.add_argument("task_dir", type=Path); command.add_argument("--expected-revision", type=int, required=True); command.add_argument("--current-ref", required=True)
    command = commands.add_parser("attest"); command.add_argument("task_dir", type=Path); command.add_argument("--attestation", required=True, type=Path)
    ...
    try:
        ...
        elif args.command == "transition": output = transition(args.task_dir, args.target, args.expected_revision, next_action=args.next_action, current_ref=args.current_ref)
        ...
        elif args.command == "refresh-ref": output = refresh_ref(args.task_dir, args.expected_revision, args.current_ref)
        elif args.command == "attest": output = attest(args.task_dir, _object(args.attestation))
