...
_MUTATING_BASH = re.compile(
    # `env -i NAME="value with spaces" git commit` and every git global-option
    # spelling (including `-C.` and `--git-dir=...`) are still writes.
    ...
    r"(?:^|[;&|]\s*)(?:(?:command|builtin)\s+|env(?:\s+(?:-[^\s]+|[A-Za-z_][A-Za-z0-9_]*=(?:\"[^\"]*\"|'[^']*'|[^\s]+)))*\s+)*(?:"
    r"git(?:\s+-[^\s]+(?:\s+(?:\"[^\"]*\"|'[^']*'|[^\s]+))?)*\s+(?:add|apply|checkout|cherry-pick|commit|merge|mv|rebase|reset|restore|rm|switch)|"
