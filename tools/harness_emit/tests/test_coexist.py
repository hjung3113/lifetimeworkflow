...
def test_all_24_commands_emit_to_both_trees(tmp_path: Path) -> None:
    """24 commands land in .opencode/command/*.md AND .claude/commands/*.md (top-level).
    ...
    assert len(opencode_cmds) == 24, f"expected 24 opencode commands, got {len(opencode_cmds)}"
    assert len(claude_cmds) == 24, f"expected 24 Claude commands, got {len(claude_cmds)}"
