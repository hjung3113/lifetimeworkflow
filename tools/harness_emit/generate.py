...
# --- Regime B managed-block bodies (shared AGENTS.md / CLAUDE.md — pointer-only, deterministic) --

# Pointer-only intro shared by both managed blocks. Deterministic (no timestamps/floats) so a
# re-emit is byte-identical and the emit-drift job stays clean on these co-owned shared files.
_MANAGED_INTRO = (
    "This block is generated by `tools.harness_emit` from the runtime-neutral `harness/` source and "
    "projected into both runtime trees (`.opencode/` + `.claude/`). Do not hand-edit — a re-emit "
    "overwrites it. Everything OUTSIDE the HARNESS-MANAGED markers is preserved verbatim."
)


def build_agents_block(
    agent_names: list[str], command_names: list[str], skill_names: list[str]
) -> str:
    """Deterministic HARNESS-MANAGED body for root ``AGENTS.md`` — a pointer index of the surface.

    Pointer-only (sorted name lists, no bodies/timestamps) so opencode reading ``AGENTS.md`` sees
    the emitted agent/command/skill inventory without the block ever flapping the drift gate.
    """
    return "\n".join(
        [
            ...
            _MANAGED_INTRO,
            "",
            f"- **Agents** (`.opencode/agent/` · `.claude/agents/`): {', '.join(agent_names)}",
            f"- **Commands** (`.opencode/command/` · `.claude/commands/`): {', '.join(command_names)}",
            f"- **Skills** (`.opencode/skill/` · `.claude/skills/`): {', '.join(skill_names)}",
            "- **Plugins** (`.opencode/plugin/`) + root `opencode.json` — see "
            "`tools/harness_emit/emit-manifest.json` for the full owned-path set.",
        ]
    )


def build_claude_block() -> str:
    """Deterministic HARNESS-MANAGED body for root ``CLAUDE.md`` — a harness pointer note.

    Static + pointer-only: the GSD ``## Project`` block and Developer Profile block in ``CLAUDE.md``
    live OUTSIDE this fence and are never touched by the splice.
    """
    return "\n".join(
        [
            "## Harness Emitter",
            ...
            _MANAGED_INTRO,
            "",
            "The opencode + Claude runtime surface (agents, commands, skills, plugins, "
            "`opencode.json`) is generated from the runtime-neutral `harness/` source by "
            "`tools.harness_emit`. See root `AGENTS.md` for the emitted index; do not hand-edit the "
...
def _merge_shared_markdown(
    ...
    """Splice the HARNESS-MANAGED block into root ``AGENTS.md`` + ``CLAUDE.md`` (Regime B, never full-write).

    Reads the CURRENT on-disk file, splices ONLY the managed block, writes the result back — the GSD
    ``## Project`` block, nearest-wins rules, and human prose OUTSIDE the fence are preserved
    verbatim (threat T-07-02). These files are NOT added to the ownership manifest (Regime B, not A).
    """
    agents_md = root / "AGENTS.md"
    if agents_md.exists():
        agents_md.write_text(
            merge.splice_managed_block(
                agents_md.read_text(encoding="utf-8"),
                build_agents_block(agent_names, command_names, skill_names),
            ),
            encoding="utf-8",
        )
    ...
    if claude_md.exists():
        claude_md.write_text(
            merge.splice_managed_block(claude_md.read_text(encoding="utf-8"), build_claude_block()),
            encoding="utf-8",
        )
...
def emit(
    ...
    # --- Regime B: managed-block MERGE into shared human/GSD files (AGENTS.md, CLAUDE.md) ---
    # Read → splice → write the HARNESS-MANAGED block ONLY; content outside the fence is preserved.
    # NOT appended to `written` → NOT in the ownership manifest (Regime B merge, not Regime A own).
    _merge_shared_markdown(
        Path(root),
        sorted(name for name, _, _, _ in agent_plan),
