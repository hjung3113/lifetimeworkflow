...
from tools.risk_router.router import DEFAULT_POLICY, RiskRouterError, decide, load_overlay, load_policy
...
def create_packet(request: object, output: str | Path, *, policy_path: str | Path = DEFAULT_POLICY, overlay_path: str | Path | None = None) -> dict[str, Any]:
    ...
    if not isinstance(task_input, dict) or set(task_input) != {"task_id", "goal", "non_goals", "acceptance_criteria", "constraints", "decision_refs"}:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Create a validated Phase-18 task packet from intake JSON")
    ...
    try:
        ...
        decision = create_packet(json.loads(raw.removeprefix("\ufeff")), args.output, policy_path=args.policy, overlay_path=args.overlay)
        sys.stdout.write(json.dumps(decision, sort_keys=True, separators=(",", ":"), ensure_ascii=True) + "\n")
    except (OSError, json.JSONDecodeError, RiskRouterError, IntakeError, PacketValidationError) as exc:
