...
def _repo(task_dir: Path) -> Path:
    root = next((parent for parent in (task_dir.resolve(), *task_dir.resolve().parents) if (parent / ".git").exists()), None)
    if root is None:
...
def _changed_paths(root: Path, baseline: str, current: str) -> list[str]:
    result = subprocess.run(["git", "-C", str(root), "diff", "--name-only", baseline, current], text=True, capture_output=True, check=False)
    if result.returncode:
...
def _atomic_write_once(path: Path, document: dict[str, Any]) -> None:
    ...
    if path.exists():
        if path.read_bytes() != payload:
            raise HandoffError("handoff already exists for a different snapshot; create a new revision first")
        return
...
def _snapshot(task_dir: Path) -> dict[str, Any]:
    ...
    artifacts = sorted(
        (
            {"evidence_id": run["id"], "path": run["artifact"]["path"], "sha256": run["artifact"]["sha256"]}
    ...
    unresolved = sorted(
        ([{"id": item["id"], "kind": "blocker"} for item in state["blockers"]]
         + [{"id": item["id"], "kind": "finding"} for item in evidence["findings"] if item["disposition"] == "open"]),
        key=lambda item: (item["kind"], item["id"]),
    )
    reads = sorted({
        _relative(root, task_dir / "task.json"), _relative(root, task_dir / "state.json"),
        _relative(root, task_dir / "evidence.json"),
        *(item["source_path"] for item in constraints), *(item["path"] for item in decision_refs),
    })
    return {
        "task_id": task["task_id"], "state_revision": state["revision"], "goal": task["goal"],
        "non_goals": task["non_goals"], "critical_constraint_ids": critical,
        "phase": state["phase"], "lane": task["lane"], "baseline": state["baseline"],
        "current_ref": state["current_ref"], "next_action": state["next_action"],
        "evidence_ids": sorted(run["id"] for run in evidence["gate_runs"]),
        "finding_ids": sorted(item["id"] for item in evidence["findings"]),
        "critical_constraint_refs": constraint_refs, "decisions": decision_refs,
        "changed_paths": _changed_paths(root, state["baseline"]["commit"], state["current_ref"]),
        "unresolved_items": unresolved,
        "stop_condition": "Stop if HANDOFF validation or /phase-gate reports refresh required.",
        "required_read_paths": reads,
        "state_ref": _path_ref(root, task_dir / "state.json"),
        "evidence_ref": _path_ref(root, task_dir / "evidence.json"), "artifact_refs": artifacts,
    }
...
def generate(task_dir: str | Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise HandoffError("sensitive handoff refused") from exc
    _validate_schema(handoff)
    _atomic_write_once(_handoff_path(packet, handoff["state_revision"]), handoff)
    return handoff


def _validate_ref(root: Path, reference: dict[str, Any], *, artifact: bool = False) -> None:
...
def validate(task_dir: str | Path, handoff_path: str | Path | None = None) -> dict[str, Any]:
    """Fail closed when the snapshot or any referenced material is stale."""
    packet = Path(task_dir); root = _repo(packet)
    ...
    _validate_schema(handoff)
    expected = _snapshot(packet)
    # Compare all snapshot fields before verifying individual path hashes, so a revision/ref
    # mismatch is never silently repaired from current state.
    if handoff != expected:
        raise HandoffError("stale handoff: regenerate from the current state revision")
    ...
    for reference in handoff["artifact_refs"]:
        ...
        _validate_ref(packet, reference, artifact=True)
    return handoff
...
def fresh_session(handoff_path: str | Path) -> dict[str, Any]:
    ...
    handoff = _load(Path(handoff_path)); _validate_schema(handoff)
    required = ("task_id", "goal", "non_goals", "critical_constraint_ids", "phase", "current_ref", "next_action")
    restored = {field: handoff[field] for field in required}
    if len(restored) != len(required) or any(value in (None, "") for value in restored.values() if not isinstance(value, list)):
...
def activate(task_dir: str | Path, state_dir: str | Path) -> dict[str, Any]:
    ...
    descriptor, temporary = tempfile.mkstemp(prefix=f".{path.name}.", suffix=".tmp", dir=Path(state_dir))
    try:
        with os.fdopen(descriptor, "wb") as handle:
            handle.write(_canonical(pointer)); handle.flush(); os.fsync(handle.fileno())
...
def resume(state_dir: str | Path, repo_root: str | Path | None = None) -> dict[str, Any]:
    ...
    pointer = _load(Path(state_dir) / ACTIVE_POINTER_NAME)
    if set(pointer) != {"task_id", "handoff_path", "state_revision"}:
        raise HandoffError("active task pointer must contain only task ID, revision, and handoff path")
    ...
    if handoff["task_id"] != pointer["task_id"] or handoff["state_revision"] != pointer["state_revision"]:
    ...
    try:
        diagnostics = phase_gate(packet, handoff["state_revision"], repo_root=root)
...
def main(argv: list[str] | None = None) -> int:
    ...
    for name in ("generate", "validate"):
        command = commands.add_parser(name); command.add_argument("task_dir", type=Path)
    fresh = commands.add_parser("fresh-session"); fresh.add_argument("handoff", type=Path)
    active = commands.add_parser("activate"); active.add_argument("task_dir", type=Path); active.add_argument("--state-dir", required=True, type=Path)
    resume_command = commands.add_parser("resume"); resume_command.add_argument("--state-dir", required=True, type=Path); resume_command.add_argument("--repo-root", type=Path)
    args = parser.parse_args(argv)
    try:
        if args.command == "generate": result = generate(args.task_dir)
        elif args.command == "validate": result = validate(args.task_dir)
        elif args.command == "fresh-session": result = fresh_session(args.handoff)
        elif args.command == "activate": result = activate(args.task_dir, args.state_dir)
        else: result = resume(args.state_dir, args.repo_root)
