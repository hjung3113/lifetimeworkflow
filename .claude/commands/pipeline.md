...
description: "Use when you need to trace or visualize the pipeline dataflow and find the owning component agent — deterministically renders the declared [[components]]/[pipeline] topology (stage list + edge chain) from tools.harness_config and resolves each stage to its component agent file. Invoke to see the end-to-end flow or to check which stage owns a contract; there is no live runtime."
---
