...
> **Gated ≠ unwatched.** Ownership of a constitution member controls *who may edit* it — the deny is
> unconditional, independent of any other read/audit practice a human chooses to layer on top.

## The other deny domain (disjoint from the constitution plane)

`path_deny_globs` carries two domains, not one, and conflating them teaches the wrong remedy:
