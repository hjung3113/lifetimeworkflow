...
# in this subdirectory is NOT counted as one of the enumerated core personas. A human or agent copies
# this file BY HAND into the active instance's own agents/ directory (the instance root is declared by
# harness/project.toml [instance] root) as <COMPONENT>.md — `/component` no longer performs that copy
# step. The derived agent's `name` equals the component id, since the conductor resolver binds
# `agents/<id>.md` (the `-engineer` suffix names THIS template file, NOT the derived per-component
# agents); fill the <PLACEHOLDER> slots from the component's [[components]] entry in project.toml.
#
# Fill every <PLACEHOLDER>: <COMPONENT> (the [[components]].id), <STAGE> (its ordinal as declared by
# the instance's own pipeline declaration), <LANG> (its `language` ref) + <TOOLCHAIN>/<TEST_CMD>
# (the matching [[languages]] toolchain),
# <CONSUMES>/<PRODUCES> (the edge contracts it reads/writes), and <BASH_SCOPE> (the
