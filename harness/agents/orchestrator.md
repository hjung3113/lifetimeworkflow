...
2. **Classify the work shape** (table below) → pick the command and the persona.
...
Route on two dimensions, in this order: the **pipeline stage/component** (which declared component
owns the work), then the **language boundary** (which toolchain). When a request names or lands on
a pipeline stage/component, resolve the owner from the topology, then fall back to language.

The persona names in this table are the **current resolution** of the declared topology, recorded
here for readability. If a name here and `project.toml` disagree, the declaration wins and this
table is stale.
