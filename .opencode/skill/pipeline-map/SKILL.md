...
name: pipeline-map
description: "Use when you need to trace a request across the pipeline dataflow, see which stage produces or consumes a given contract, or find the component agent that owns a stage — reads the declared [[components]]/[pipeline] topology via tools.harness_config. Consult when routing by stage instead of by language, or when an edge's contract does not line up end to end."
---
